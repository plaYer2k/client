...
void ClientModeASW::FireGameEvent( IGameEvent *event )
{
	...
	else if ( Q_strcmp( "game_newmap", eventname ) == 0 )
	{
		...
		char	szMap[256];							// p2k map specific config loader
		char	szExecMap[256];							//
		Q_strncpy( szMap, engine->GetLevelName(), sizeof( szMap ) );		// get the mapname
		Q_StripExtension( szMap, szMap, sizeof( szMap ) );			// get rid of the extension
		Q_snprintf( szExecMap, sizeof(szExecMap), "exec configloader/%s.cfg\n", szMap );	// form the command
		engine->ClientCmd_Unrestricted( szExecMap );				// run the command
	}
