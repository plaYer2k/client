	...
	else if ( !Q_stricmp( command, "AcceptButton" ) )
	{
		...
		if ( pHighlighted )
		{
			...
			if ( !Briefing()->IsProfileSelectedBySomeoneElse( nProfileIndex ) )
			{
				...
				// p2k character specific and class specific config loader
				CASW_Marine_Profile* pProfile = Briefing()->GetMarineProfileByProfileIndex( nProfileIndex );
				if ( pProfile )
				{
					char exec[256];
					int classIndex = nProfileIndex;
					while (classIndex > 3) { classIndex -= 4; }
					Q_snprintf( exec, sizeof(exec), "exec /configloader/chars/%s.cfg\nexec /configloader/chars/%s.cfg", pProfile->m_PortraitName , (classIndex==0?"Officer":(classIndex==1?"SpecialWeapons":(classIndex==2?"Medic":"Tech"))));
					engine->ClientCmd_Unrestricted( exec );
				}
			}
			else
			{
